---
title: "AFLM Season Predictions"
description: "ELO ratings and chances of making various finals for every team in the AFLM competition"
execute:
  freeze: false
page-layout: full
---
...
results        <- read_csv(file = paste0(data_url, "AFLM_results.csv"))
sims_combined  <- read_csv(file = paste0(data_url, "AFLM_sims_combined.csv"))
ongoing_status <- read_csv(file = paste0(data_url, "AFLM_home_away_ongoing.csv"))
elo            <- read_csv(file = paste0(data_url, "AFLM_elo.csv"))

home_away_ongoing <- ongoing_status$home_away_ongoing[1]
```

```{r prep, include=FALSE}
forecasts2 <- sims_combined %>%
  mutate(
    points   = Wins,
    team     = Team,
    group    = Season,
...
new_season <- forecasts2$Round[1] == 0 | forecasts2$Round[1] == 28

results <- results %>% slice_tail()
...
if (results$Round == 27 & !new_season) {
  ...
  forecasts2 <- forecasts2 %>%
    ...
    mutate(
      elo.old = ifelse(is.na(ELO_pre), elo.old, ELO_pre),
...
group_cols <- c("Wins", "Top.1", "Top.4", "Top.10")

# CHANGED: added make_elim as the first Finals Chances column
knockout_cols <- c("make_qf_ef", "make_semis", "make_prelim", "make_gf", "win_league")
  ...
  columns = list(
    ...
    team = colDef(
      ...
      cell = function(value, index) {
        div(
          class = "team",
          img(class = "flag", alt = paste(value, "flag"), src = paste0("../img/", value, ".svg")),
    ...
    # CHANGED: "Make Finals" → "Top 10" to reflect new meaning (making wildcard round)
    Top.10 = knockout_column(name = "Top 10"),

    # CHANGED: make_elim is new first column in Finals Chances, gets the border-left
    make_qf_ef = knockout_column(name = "Make QF/EF", maxWidth = 90, class = "border-left"),

    # CHANGED: border-left removed from make_semis (moved to make_elim above)
    make_semis  = knockout_column(name = "Make Semis",  maxWidth = 90),
...
rnd  <- sims_combined$Round %>% max()
seas <- sims_combined$Season %>% max()
